...
Route::middleware([$middleAuth, "throttle:60,1", "disabled"])->group(function () {
    Route::middleware("verified", "throttle:60,1")->group(function () {
        ...
        #region Genders
        Route::get("/user/genders", [GenderController::class, "searchGenders"]);
        Route::post("/gender", [GenderController::class, "addGender"]);
        Route::put("/gender/{gender}", [GenderController::class, "updateGender"]);
        Route::patch("/gender/{gender}", [GenderController::class, "toggleGender"]);
        #endregion

        #region Logs
        Route::get("/logsy/{year}/{month?}/{day?}", [LogsController::class, "getLogs"]);
        Route::get("/log/levels", [LogsController::class, "getLevels"]);
        Route::get("/log/system", [LogsController::class, "getSystemLog"]);
        Route::get("/log/system/dl", [LogsController::class, "downloadLogFile"]);
        Route::delete("/log/system", [LogsController::class, "clearLogFile"]);
        #endregion

        #reqion PrivacyPolicy Management
        Route::post("/policy/privacy", [
            PrivacyPolicyController::class,
            "createPrivacyPolicy",
        ])->name("privacy.create");

        Route::post("/policy/activate/{privacy}", [
            PrivacyPolicyController::class,
            "activatePrivacyPolicy",
        ])->name("privacy.activate");

        Route::get("/policy/privacies", [PrivacyPolicyController::class, "getPolicies"])->name(
            "privacy.list"
        );
        Route::get("/policy/privacy/{privacy}", [
            PrivacyPolicyController::class,
            "getPolicy",
        ])->name("privacy.get");
        #endregion

        Route::get("/sample/pdf", fn() => response()->file(storage_path("app\\public\\sample.pdf")));
    });
    ...
    Route::prefix('par')->group(function(){
        Route::get('/{par}', [ParController::class, 'show']);
        Route::get("/", [ParController::class, 'list']);
        Route::post('/', [ParController::class, 'create']);
    });

    Route::prefix('custodian')->group(function(){
        Route::get('/{custodian}', [CustodianController::class,'show']);
        Route::get("/", [CustodianController::class, 'list']);
        Route::post('/', [CustodianController::class, 'create']);
        Route::put('/{custodian}', [CustodianController::class, 'update']);
        Route::delete('/{custodian}', [CustodianController::class, 'delete']);
        Route::patch('/{custodian}', [CustodianController::class, 'restore']);
        Route::post('/bind-par/{custodian}', [CustodianController::class, 'bindPar']);
        Route::delete('/remove-par/{custodian}/{par}', [CustodianController::class, 'removePar']);
    });
    #endregion
});
...
Route::middleware("api", "throttle:60,1")->group(function () {
    ...
    Route::post("/password/forgot", [PasswordController::class, "forgot_password"])->name(
        "password.email"
    );
    Route::post("/password/reset", [PasswordController::class, "change_password"])->name(
        "password.update"
    );
    #endregion
    ...
    #region Address
    Route::get("/address/initial/barangay/{Code}", [
    ...
    #region policies
    Route::get("/policy/prompt", [PrivacyPolicyController::class, "getPublicPolicy"])->name(
        "policy.prompt"
    );
    Route::get("/policy/public", [PrivacyPolicyController::class, "getPublicPolicy"])->name(
        "policy.public"
    );
    #endregion
});
...
Route::middleware("api")->group(function () {
    # Images
    Route::get("/image/{thumbsize}/{hash}", [ImagesController::class, "display_thumb"])->name(
        "image.thumb"
    );
