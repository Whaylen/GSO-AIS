        ...
        >
          ...
          <div v-else class="flex min-h-[19.7rem] flex-auto flex-col gap-1">
            <div class="flex items-center gap-2">
              ...
              <TButton
                :label="addImage.mode == 'upload' ? 'Take Photo' : 'Upload'"
                :icon="
                  addImage.mode == 'upload' ? 'photo_camera' : 'file_upload'
                "
                class="rounded-lg px-3 py-1"
                @click="
                  addImage.mode =
                    addImage.mode == 'upload' ? 'camera' : 'upload'
                "
              />
              <TButton
                icon="save"
            ...
            <div class="flex flex-auto items-start justify-center">
              ...
              <TCamera
                v-else
                v-model="addImage.file"
                wrapperClass="h-[17rem] w-full rounded-lg"
              />
            </div>
          </div>
        </transition>
