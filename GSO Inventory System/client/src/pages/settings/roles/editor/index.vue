<template>
  ...
  >
    ...
    <div class="flex flex-auto flex-col md:flex-row md:items-stretch">
      <div class="md:w-64">
        <div
          class="sticky top-0 md:w-64"
          :class="{
            'top-0': !systemStore.settings.navbar.fixed,
            'top-14': systemStore.settings.navbar.fixed,
          }"
        >
          <div class="px-2">
            <TButton
      ...
      >
        <div class="z-10 border-b p-2">
          ...
          >
            ...
            <template #error><span class="hidden"></span></template>
          </TInput>
...
const onWrapperResize = (size) => {
  let cols = Math.floor(size.width / 256);
  permissionsWrapper.value.style.setProperty("--columns", cols);
};

const searchPermissions = (initialNone = false) => {
  _loading.value = true;
...
onMounted(() => {
  searchPermissions(!props.modelValue?.id);
});
</script>
