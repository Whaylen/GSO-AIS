<template>
  ...
  >
    ...
    >
      <div
        class="absolute inset-x-0 top-0 z-10 flex select-none items-center justify-center text-sm font-bold uppercase transition-all md:text-base"
        :class="[
          files?.length <= 0 && 'bottom-0 flex-col',
          files?.length > 0 &&
            'rounded-lg border-b border-slate-500/50 px-2 py-2',
        ]"
      >
    ...
    <transition-group
      tag="div"
      name="files"
      class="relative flex max-h-full flex-auto flex-col gap-1 overflow-y-auto overflow-x-hidden"
...
<script>
const ACCEPT_CHECK_REG = /^\.[a-zA-Z0-9]{3,4}(,\.[a-zA-Z0-9]{3,4})*$/;
</script>
