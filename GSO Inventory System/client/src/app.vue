<template>
  <TRicked v-if="devt" class="h-screen w-screen items-center" />
  <template v-else>
    <router-view />
    <teleport to="body">
...
const DevtBlocked = defineAsyncComponent(() =>
  import("@/components/others/devtBlocked.vue")
);
const InstallPrompt = defineAsyncComponent(() =>
  import("@/plugins/pwa/installPrompt.vue")
);
...
const CookiePrompt = defineAsyncComponent(() =>
  import("@/pages/settings/privacy_policy/preview/prompt.vue")
);
</script>
